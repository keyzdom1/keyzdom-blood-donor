...
function AnalyticsContent() {
  ...
  useEffect(() => {
    if (token) {
      api.get("/api/admin/analytics", token).then((data) => {
  ...
  if (!analytics) {
    return (
      <div className="text-center text-text-secondary py-12">Failed to load analytics</div>
    );
  }
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        >
          <div className="p-5 border-b border-border">
            <h2 className="text-lg font-semibold text-text-primary">Requests by Status</h2>
          </div>
                ...
                <div className="flex items-center gap-3">
                  <div className="w-32 h-2 bg-border rounded-full overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${analytics.total_requests > 0 ? (count / analytics.total_requests) * 100 : 0}%`,
                      }}
                    />
                  </div>
        ...
        >
          <div className="p-5 border-b border-border">
            <h2 className="text-lg font-semibold text-text-primary">Requests by Urgency</h2>
          </div>
          <div className="p-5 space-y-3">
              ...
              return (
                <div key={urgency} className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-3">
                    <div className="w-32 h-2 bg-border rounded-full overflow-hidden">
                      <div
                        className={`h-full ${colors[urgency] || "bg-primary"} rounded-full transition-all`}
                        style={{
                          width: `${analytics.total_requests > 0 ? (count / analytics.total_requests) * 100 : 0}%`,
                        }}
                      />
        ...
        >
          <div className="p-5 border-b border-border">
            <h2 className="text-lg font-semibold text-text-primary">Match Outcomes</h2>
          </div>
          <div className="p-5 space-y-3">
        ...
        >
          <div className="p-5 border-b border-border">
            <h2 className="text-lg font-semibold text-text-primary">Donor Availability</h2>
          </div>
            ...
            <div className="relative w-32 h-32">
              <svg className="w-32 h-32 -rotate-90" viewBox="0 0 120 120">
                ...
                <circle
                  ...
                  stroke="currentColor"
                  strokeWidth="10"
                  strokeDasharray={`${(analytics.donor_availability_rate / 100) * 314} 314`}
                  className="text-primary"
                />
              </svg>
              <div className="absolute inset-0 flex items-center justify-center">
                <span className="text-2xl font-bold text-primary">{analytics.donor_availability_rate}%</span>
              </div>
            </div>
