...
function OrgsContent() {
  ...
  const fetchPending = () => {
    ...
    api.get("/api/admin/organizations", token).then((data) => {
      setPendingOrgs(data as OrgInfo[]);
      setLoading(false);
  ...
  const verify = async (orgId: number) => {
    if (!token) return;
    await api.post(`/api/admin/organizations/${orgId}/verify`, {}, token);
    fetchPending();
  };

  const reject = async (orgId: number) => {
    if (!token) return;
    await api.post(`/api/admin/organizations/${orgId}/reject`, {}, token);
    fetchPending();
  };
      ...
      >
        ...
        {loading ? (
          ...
          <div className="p-12 text-center">
            ...
            <p className="text-text-secondary">No organizations pending verification.</p>
          </div>
        ) : (
            ...
            {pendingOrgs.map((org, i) => (
              ...
              >
                <div>
                  ...
                  <div className="text-sm text-text-secondary capitalize">{org.type.replace("_", " ")}</div>
                </div>
                <div className="flex gap-2">
                  <button
                    onClick={() => verify(org.id)}
                    className="bg-success hover:bg-success/80 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors flex items-center gap-1"
                  >
                  ...
                  <button
                    ...
                    className="bg-surface border border-border hover:bg-destructive/10 text-text-primary px-4 py-2 rounded-lg text-sm font-medium transition-colors flex items-center gap-1"
                  >
